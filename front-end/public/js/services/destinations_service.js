app.factory('destinationsService', function () {
  return {
    all: [{
        "id":1,
        "city":"London",
        "country":"United Kingdom",
        "image_url":"https://upload.wikimedia.org/wikipedia/commons/6/6e/London_Thames_Sunset_panorama_-_Feb_2008.jpg",
        population: "8,538,689",
        languages: "English",
        sights: "London Eye, Tower of London, Tower Bridge, Westminster Abbey, Buckingham Palace, British Museum",
        notes: ""
      },{
        "id":2,
        "city":"Bangkok",
        "country":"Thailand",
        "image_url":"http://static.asiawebdirect.com/m/bangkok/portals/bangkok-com/shared/teasersL/TOP10/top-10-best-hotels/teaserMultiLarge/image/top10-hotels-.jpg",
        population: "8,280,925",
        languages: "Thai",
        sights: "Grand Palace, Wat Pho, Wat Phra Kaew, Wat Arun",
        notes: ""
      },{
        "id":3,
        "city":"Paris",
        "country":"France",
        "image_url":"http://s.petrolicious.com/2015/vintage-friday/08-aug/paris/paris-6.jpg",
        population: "2,240,621",
        languages: "French",
        sights: "Eiffel Tower, The Louvre, Notre Dame de Paris, Arc de Triomphe, Montmartre, Musee d'Orsay",
        notes: ""
      },{
        "id":4,
        "city":"Dubai",
        "country":"United Arab Emirates",
        "image_url":"https://i.ytimg.com/vi/S1oX9L6e6_Q/maxresdefault.jpg",
        population: "2,459,068",
        languages: "Arabic",
        sights: "Burj Khalifa, Burj Al Arab, Wild Wadi Water Park, Ski Dubai, Palm Jumeirah",
        notes: ""
      },{
        "id":5,
        "city":"Istanbul",
        "country":"Turkey",
        "image_url":"http://www.betches.com/sites/default/files/article/list/images/istanbul.jpg",
        population: "14,025,646",
        languages: "Turkish",
        sights: "Hagia Sophia, Sultan Ahmed Mosque, Topkapi Palace, Basilica Cistern, Galata Tower, Suleymaniye Mosque",
        notes: ""
      },{
        "id":6,
        "city":"New York City",
        "country":"United States",
        "image_url":"http://cdn1.tnwcdn.com/wp-content/blogs.dir/1/files/2013/09/nyc.jpg",
        population: "8,491,079",
        languages: "English",
        sights: "Central Park, Empire State Building, Statue of Liberty, Metropolitan Museum of Art, Times Square",
        notes: ""
      },{
        "id":7,
        "city":"Singapore",
        "country":"Singapore",
        "image_url":"http://d3c6l3uum4x5po.cloudfront.net/wp-content/uploads/2015/02/Singapore-city-branch-image1.jpg",
        population: "5,535,000",
        languages: "English, Malay, Mandarin, Tamil",
        sights: "Sentosa, Singapore Flyer, Universal Studios Singapore, Night Safari, Singapore Botanic Gardens, Jurong Bird Park",
        notes: ""
      },{
        "id":8,
        "city":"Kuala Lumpur",
        "country":"Malaysia",
        "image_url":"https://higherlogicdownload.s3.amazonaws.com/SPE/6273cee3-81ec-4728-a813-68cacef01783/UploadedImages/KL-Skyline_Night_HDR.JPG",
        population: "1,768,000",
        languages: "Malaysian, (recognized: English)",
        sights: "Petronas Towers, Bukit Bintang, Aquaria KLCC, Kuala Lumpur Bird Park, Islamic Arts Museum Malaysia, Merdeka Square",
        notes: ""
      },{
        "id":9,
        "city":"Seoul",
        "country":"South Korea",
        "image_url":"http://www.layoverguide.com/wp-content/uploads/2011/06/Seoul-South-Korea.jpg",
        population: "10,117,909",
        languages: "Korean",
        sights: "Gyeongbogung, North Seoul Tower, Myeong-dong, Cheonggyecheon, War Memorial of Korea, National Museum of Korea",
        notes: ""
      },{
        "id":10,
        "city":"Hong Kong",
        "country":"China",
        "image_url":"http://www.happycow.net/blog/wp-content/uploads/2015/09/hongkong1.jpg",
        population: "7,234,800",
        languages: "Chinese, English, (recognized: Cantonese)",
        sights: "Ocean Park, Victoria Peak, Disneyland Hong Kong, Lantau Island, Tian Tan Buddha, Ngong Ping 360",
        notes: ""
    }]
  }
});
